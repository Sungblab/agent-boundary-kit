...
const fallbackTrapPattern = /pymupdf[-_.]?fallback/i;
const forbiddenLinePatterns = [
  {
    label: "PyMuPDF fallback module reference",
    display: "pymupdf-fallback",
    regex: /pymupdf[-_.]?fallback/i,
  },
  {
    label: "PyMuPDF fallback call",
    display: "extractWithPyMuPdf",
    regex: /extractWithPyMuPdf/i,
  },
  {
    label: "hardcoded parser output",
    display: "Fixture PDF parsed by opendataloader-pdf",
    regex: /Fixture PDF parsed by opendataloader-pdf/,
  },
];

function usage() {
  process.stderr.write("Usage: node benchmarks/scripts/scan-parser-fallback-boundary.js <file-or-repo> [...]\n");
}
...
function scanFile(filePath) {
  ...
  if (path.basename(filePath).toLowerCase() === "pdf-ingest.js" && /function\s+parsePdf|parsePdf\s*=/m.test(source)) {
    if (!/runOpenDataLoader/.test(source)) {
      findings.push(
        lineFinding(
          filePath,
          1,
          "selected parser bypass",
          "missing runOpenDataLoader",
          "pdf-ingest.js parsePdf does not call the selected parser path"
        )
      );
    }
  }
...
function main() {
  ...
  process.stdout.write("parser-fallback-boundary-scan found forbidden parser fallback behavior:\n");
