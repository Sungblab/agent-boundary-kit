...
const scannerCatalog = {
  ...
  "approved-file-mask-scan": {
    ...
    inputsRequired: ["repoRoot", "changedFiles", "approvedScope"],
    output: {
      clearReason: "no approved file mask finding for declared inputs",
      findingReason: "approved file mask violation found",
      findingRule: "approved-file-mask",
    },
  },
  "research-mode-no-write-scan": {
    script: "benchmarks/scripts/scan-research-mode-no-write.js",
    inputsRequired: ["repoRoot", "task"],
    output: {
      clearReason: "no research-mode no-write artifact finding for declared inputs",
      findingReason: "research-mode no-write artifact found",
      findingRule: "research-mode-no-write",
    },
  },
  "untrusted-context-canary-scan": {
...
function readJsonIfExists(relativePath) {
  ...
  if (!fs.existsSync(fullPath)) {
    return { present: false, parsed: null, error: null };
  }
  try {
    return {
      present: true,
    ...
    return { present: true, parsed: null, error: error.message };
  }
}

function fileStatus(relativePath, validate) {
  const fullPath = path.join(root, relativePath);
  if (!fs.existsSync(fullPath)) {
    return { path: toDisplayPath(relativePath), status: "missing", ok: false };
  }
  if (!validate) {
    return { path: toDisplayPath(relativePath), status: "present", ok: true };
  }
...
function validateJsonFile(relativePath, validateParsed) {
  ...
  return validateParsed(result.parsed);
}

function validateCodexManifest(relativePath) {
  return validateJsonFile(relativePath, (manifest) => {
    if (manifest.name !== "agent-boundary-kit") {
      return { ok: false, reason: "manifest name must be agent-boundary-kit" };
    }
    if (manifest.skills !== "./skills/") {
      return { ok: false, reason: "manifest must package ./skills/" };
    }
    const mcpServers = manifest.mcpServers || manifest.mcp_servers;
    if (!mcpServers || !mcpServers.abk || mcpServers.abk.command !== "abk-mcp-server") {
      return { ok: false, reason: "manifest must declare abk-mcp-server" };
    }
    if (manifest.hooks !== "./hooks/hooks.json") {
      return { ok: false, reason: "manifest must point at ./hooks/hooks.json" };
    }
    return { ok: true };
  });
}
...
function validateMarketplace(relativePath) {
  return validateJsonFile(relativePath, (marketplace) => {
    const plugins = Array.isArray(marketplace.plugins) ? marketplace.plugins : [];
    const entry = plugins.find((plugin) => plugin.name === "agent-boundary-kit");
    if (!entry) {
      return { ok: false, reason: "marketplace must expose agent-boundary-kit" };
    }
    if (!entry.source || entry.source.source !== "local" || entry.source.path !== "./plugins/codex-agent-boundary-kit") {
      return { ok: false, reason: "marketplace source must point at ./plugins/codex-agent-boundary-kit" };
    }
    return { ok: true };
  });
}

function validateHooks(relativePath) {
  return validateJsonFile(relativePath, (hooks) => {
    if (!hooks || typeof hooks !== "object" || Array.isArray(hooks) || !hooks.hooks) {
      return { ok: false, reason: "hooks file must contain hooks object" };
    }
    return { ok: true };
  });
}
...
function harnessChecks() {
  return [
    fileStatus(".agents/plugins/marketplace.json", validateMarketplace),
    fileStatus("plugins/codex-agent-boundary-kit/.codex-plugin/plugin.json", validateCodexManifest),
    fileStatus("plugins/codex-agent-boundary-kit/skills/boundary-check/SKILL.md"),
    fileStatus("plugins/codex-agent-boundary-kit/hooks/hooks.json", validateHooks),
    fileStatus("plugins/claude-code-agent-boundary-kit/plugin.json"),
    fileStatus("plugins/claude-code-agent-boundary-kit/skills/abk-boundary-check/SKILL.md"),
    fileStatus("bin/abk-mcp-server.js"),
  ];
}

function createHarnessOutput(command) {
  const checks = harnessChecks();
  const ok = checks.every((check) => check.ok);
  return {
    tool: `harness ${command}`,
    status: ok ? "ok" : "needs-attention",
    ok,
    product: {
      problem:
        "AI coding agents cross boundaries when they solve the wrong problem, add fallback before root cause, mutate tests to pass, trust untrusted evidence, edit outside scope, or claim completion without verification.",
      role: "ABK is a preflight and finish guard for fixture-backed boundary failures, not a project-management or handoff system.",
      devflowBoundary:
        "Use Devflow for repo-local work state, handoffs, and recorded gate/review evidence. Use ABK to decide whether the agent is about to cross a known boundary and which read-only scanner evidence applies.",
    },
    checks,
    install: {
      preferredCodexMarketplaceCommand: "codex plugin marketplace add Sungblab/agent-boundary-kit",
      localCodexMarketplaceCommand: "codex plugin marketplace add ./",
      nextStep: "Restart Codex, open Plugins or /plugins, install Agent Boundary Kit, then start a new thread.",
      note: "Plugin installation is the reusable Codex surface. Repo-local skills remain useful for authoring and fixture development.",
    },
  };
}

function harnessPlan() {
  const output = createHarnessOutput("plan");
  return {
    ...output,
    actions: [
      {
        id: "codex-marketplace",
        status: output.ok ? "ready" : "blocked-until-health-ok",
        command: "codex plugin marketplace add Sungblab/agent-boundary-kit",
        reason: "Register the repo marketplace so users can install the plugin once instead of copying per-repo files.",
      },
      {
        id: "codex-plugin-install",
        status: "manual-review-required",
        command: "Open Plugins or /plugins, select Agent Boundary Kit, then install and enable it.",
        reason: "Codex plugin install and hook trust are user-owned review steps.",
      },
      {
        id: "new-thread",
        status: "manual",
        command: "Start a new Codex thread and ask for Agent Boundary Kit or @agent-boundary-kit.",
        reason: "Codex loads newly installed plugin skills and MCP servers in fresh sessions.",
      },
    ],
  };
}

function renderHarnessText(output) {
  const lines = [
    `Agent Boundary Kit harness: ${output.status}`,
    "",
    "Role:",
    `- ${output.product.role}`,
    `- ${output.product.devflowBoundary}`,
    "",
    "Checks:",
    ...output.checks.map((check) => {
      const reason = check.reason ? ` (${check.reason})` : "";
      return `- ${check.ok ? "ok" : "fail"} ${check.path}${reason}`;
    }),
    "",
    "Codex install:",
    `- ${output.install.preferredCodexMarketplaceCommand}`,
    "- Restart Codex, open Plugins or /plugins, install Agent Boundary Kit, then start a new thread.",
  ];
  if (output.actions) {
    lines.push("", "Plan:", ...output.actions.map((action) => `- ${action.id}: ${action.command}`));
  }
  return `${lines.join("\n")}\n`;
}
...
function harness(command, args) {
  ...
  let output = command === "plan" || command === "install" ? harnessPlan() : createHarnessOutput(command);

  if (command === "install" && args.includes("--confirm")) {
    const result = spawnSync("codex", ["plugin", "marketplace", "add", "Sungblab/agent-boundary-kit"], {
      cwd: root,
      encoding: "utf8",
    });
    output = {
      ...
      installCommand: "codex plugin marketplace add Sungblab/agent-boundary-kit",
      installStatus: result.status === 0 ? "marketplace-registered" : "failed",
      installExitCode: result.status === null ? 2 : result.status,
      installStdout: result.stdout ? result.stdout.trim() : "",
      installStderr: result.stderr ? result.stderr.trim() : "",
    };
    if (result.status !== 0) {
      output.status = "install-failed";
      output.ok = false;
    }
    ...
    output = {
      ...
      nextCommand: "agent-boundary-kit harness install --confirm",
    };
  }

  if (json) {
    writeJson(output, output.ok ? 0 : 1);
    return;
  }
  process.stdout.write(renderHarnessText(output));
...
function selectScanners(input) {
  ...
  if (input.hookId === "post_edit_scope_check") {
    ...
    if (
      hasAnyText(inputs.approvedScope) &&
      taskSummaryMatches(input.task, /approved file mask|approved edit mask|declared approved file mask/i)
    ) {
      selected.push("approved-file-mask-scan");
    }
    if (taskSummaryMatches(input.task, /research mode|no-write|final-response-only/i)) {
      selected.push("research-mode-no-write-scan");
    }
...
function scannerArgs(input, scanner) {
  ...
  if (scanner === "approved-file-mask-scan") {
    const masks = input.inputs.approvedScope || [];
    return [
      "--repo-root",
      resolveRepoRootArg(input),
      ...masks.flatMap((mask) => ["--approved-mask", mask]),
      ...resolveChangedFileArgs(input),
    ];
  }

  if (scanner === "research-mode-no-write-scan") {
    return ["--repo-root", resolveRepoRootArg(input)];
  }
...
function main(args = process.argv.slice(2)) {
  ...
  if (command === "harness" && ["inspect", "plan", "install", "health"].includes(flag)) {
    harness(flag, args.slice(2));
    return;
  }
