...
5. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. The first ten reviewed results are recorded under `benchmarks/results/`; synthesize those results into enforcement-surface gaps before adding new hooks or packaging. Refine `templates/AGENTS.boundary.md`, `templates/CLAUDE.boundary.md`, `skills/boundary-check/SKILL.md`, and `hooks/claude/*` only from fixture evidence.
