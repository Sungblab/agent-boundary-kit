...
5. Review `docs/first-ten-result-synthesis.md`, `docs/first-scanner-candidate.md`, `docs/scanner-validation-test-runtime-patch.md`, `docs/scanner-application-test-runtime-patch.md`, `benchmarks/scripts/scan-test-runtime-patch.js`, `skills/boundary-check/SKILL.md`, `templates/AGENTS.boundary.md`, `templates/CLAUDE.boundary.md`, and `hooks/claude/*`. Apply `test-runtime-patch-scan` to the next E2E or browser-style test-integrity run before adding another scanner pattern. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. Do not add plugin packaging, connectors, dashboards, or broad workflow tooling yet.
