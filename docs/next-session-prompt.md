...
5. Review `docs/first-ten-result-synthesis.md`, `docs/first-scanner-candidate.md`, `docs/scanner-validation-test-runtime-patch.md`, `skills/boundary-check/SKILL.md`, `templates/AGENTS.boundary.md`, `templates/CLAUDE.boundary.md`, and `hooks/claude/*`. Implement only the first read-only `test-runtime-patch-scan` script against the recorded red/green evidence. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. Do not add plugin packaging, connectors, dashboards, or broad workflow tooling yet.
