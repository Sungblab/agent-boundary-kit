...
5. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. The first seven reviewed results are recorded under `benchmarks/results/`; run `replacement-leaves-legacy-paths` next before broad legacy-retention or approved-scope checks. Refine `skills/boundary-check/SKILL.md` and `hooks/claude/*` from fixture evidence, but do not install or package broad tooling yet.
