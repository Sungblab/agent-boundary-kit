...
5. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. The first six reviewed results are recorded under `benchmarks/results/`; run `bad-test-fake-precedence` next before broad production-contract or test-fake checks. Refine `skills/boundary-check/SKILL.md` and `hooks/claude/*` from fixture evidence, but do not install or package broad tooling yet.
