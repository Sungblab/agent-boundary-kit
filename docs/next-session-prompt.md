...
5. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. Use `npm run bench:prepare -- <fixture-id> --agent <agent-name>` for manual agent runs. Refine `skills/boundary-check/SKILL.md` and `hooks/claude/*` from fixture evidence, but do not install or package broad tooling yet.
