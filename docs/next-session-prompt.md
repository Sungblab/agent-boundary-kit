...
5. Review `docs/first-ten-result-synthesis.md`, then refine `templates/AGENTS.boundary.md`, `templates/CLAUDE.boundary.md`, `skills/boundary-check/SKILL.md`, and `hooks/claude/*` only where the synthesis identifies a fixture-backed gap. Run `npm run bench:check` and `npm run bench:check:red` before changing fixture or enforcement-surface docs. Do not add new hook scripts, plugin packaging, connectors, dashboards, or broad workflow tooling yet.
