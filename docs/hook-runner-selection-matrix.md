...
| `post_edit_scope_check` | `hooks/claude/post-edit-scope-check.md` | Research mode or final-response-only work declares no-write task metadata. | `research-mode-no-write-scan` | `benchmarks/scripts/scan-research-mode-no-write.js` | `repoRoot`, `task` |
