...
The first read-only execution contract is recorded in `docs/hook-runner-read-only-execution-contract.md`. Its local implementation is split between `bin/abk-runner.js` and `lib/abk-runner-core.js`, limited to executing `parser-fallback-boundary-scan`, `latex-renderer-boundary-scan`, `hardcoded-credential-fallback-scan`, and `legacy-surface-retention-scan` from explicit runner input and changed file paths, `test-runtime-patch-scan` from explicit runner input and test file paths, `test-fake-contract-scan` from explicit runner input plus test and production file paths, `completion-evidence-gate-scan` from explicit runner input plus the declared repo root, and `untrusted-context-canary-scan` from explicit runner input plus changed files or completion draft path.
