...
The first read-only execution contract is recorded in `docs/hook-runner-read-only-execution-contract.md`. Its local implementation is split between `bin/abk-runner.js` and `lib/abk-runner-core.js`, limited to executing `parser-fallback-boundary-scan`, `latex-renderer-boundary-scan`, and `legacy-surface-retention-scan` from explicit runner input and changed file paths, `test-runtime-patch-scan` from explicit runner input and test file paths, and `test-fake-contract-scan` from explicit runner input plus test and production file paths.
