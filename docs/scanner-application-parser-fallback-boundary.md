...
| `hardcoded-fallback-secret` | exit 0 | Correct non-match; credential fallback needs a separate scanner. |
| `latex-pdf-tool-boundary` | exit 0 | Correct non-match; renderer fallback needs a separate scanner. |
