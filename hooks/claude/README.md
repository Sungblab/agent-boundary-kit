...
The first read-only execution contract is recorded in `docs/hook-runner-read-only-execution-contract.md`. Its local implementation is split between `bin/abk-runner.js` and `lib/abk-runner-core.js`, limited to `legacy-surface-retention-scan` with explicit runner input and changed file paths, and `test-runtime-patch-scan` with explicit runner input and test file paths. It is still not an installed hook.
